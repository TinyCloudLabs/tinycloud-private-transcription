...
const range = (sequence: number, speaker_key: string, speaker_name: string, start_ms: number, end_ms: number, source: "glow-bound" | "provisional" = "glow-bound", voiced = true) => {
  ...
  return { version: 1 as const, meeting_id: "m", sequence, idempotency_key: `r${sequence}`, speaker_key, speaker_name,
    ...
    sample_rate: 16000, channels: 1 as const, byte_count: bytes.byteLength, sha256: createHash("sha256").update(bytes).digest("hex"),
    state: "uploaded" as const, url: `/attributed-audio/m/${sequence}`, bytes };
};
test("closed evidence batches contiguous named speakers and maps text without diarization", async () => {
  ...
  const manifest: AttributedManifest = { version: 1, meeting_id: "m", state: "closed", ranges: [a, b, c] };
  expect(attributedBatches(manifest)).toHaveLength(2);
  const transcript = await transcribeAttributedManifest(manifest, async item => ({ 0: a, 1: b, 2: c }[item.sequence]!.bytes), async (_pcm, batch) => ({ text: batch.speaker_name === "Alice" ? "hello" : "world" }), "en");
...
test("short voiced evidence is retained, silence avoids a paid call, and provisional stays provisional", async () => {
  ...
  const manifest: AttributedManifest = { version: 1, meeting_id: "m", state: "closed", ranges: [short, provisional, silent] };
  let calls = 0;
  const transcript = await transcribeAttributedManifest(manifest, async item => ({ 0: short, 1: provisional, 2: silent }[item.sequence]!.bytes), async (_pcm, batch) => { calls++; return { text: batch.speaker_name }; }, "en");
  expect(calls).toBe(2);
...
test("open, failed, absolute, and oversized evidence are rejected before fetch", () => {
  ...
  expect(() => attributedBatches({ version: 1, meeting_id: "m", state: "closed", ranges: [{ ...one, state: "failed" }] })).toThrow();
  expect(() => attributedBatches({ version: 1, meeting_id: "m", state: "closed", ranges: [{ ...one, url: "s3://bucket/x" }] })).toThrow();
  expect(() => attributedBatches({ version: 1, meeting_id: "m", state: "closed", ranges: [range(0, "a", "Alice", 0, 121000)] })).toThrow();
});
