...
test("VEXA_MAX_TIME_LEFT_ALONE_MS accepts only positive integers", () => {
  ...
  try {
    ...
    expect(positiveIntegerEnv("VEXA_MAX_TIME_LEFT_ALONE_MS", "60000")).toBe(60_000);
    for (const value of ["0", "-1", "1.5", "bad"]) {
      process.env.VEXA_MAX_TIME_LEFT_ALONE_MS = value;
      expect(() => positiveIntegerEnv("VEXA_MAX_TIME_LEFT_ALONE_MS", "60000")).toThrow("VEXA_MAX_TIME_LEFT_ALONE_MS must be a positive integer");
    }
  } finally {
