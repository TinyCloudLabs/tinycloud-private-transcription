...
For every dispatched meeting, the worker sends Vexa `automatic_leave.max_time_left_alone` from `VEXA_MAX_TIME_LEFT_ALONE_MS` (default 300000 milliseconds). This per-meeting override releases the bot and continues normal finalization as `completed(left_alone)` five minutes after everyone else leaves on Jitsi or Google Meet; operators can tune the delay when needed.
